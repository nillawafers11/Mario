...
{
    public class BridgeSegment : Block
    {
        public BridgeSegment(Vector2 position, Vector2 speed, ItemType itemType, int itemCount, bool ishidden)
            : base(position, speed, itemType, itemCount, ishidden)
        {
            Sprite = BlockSpriteFactory.Instance.CreateSprite(position, (int)BlockSpriteFactory.eBlockSprite.BridgeSegment);
